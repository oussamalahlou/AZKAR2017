...
// ---------------------------------
 ...
 // Retourne une date 00:00:00:00
  exports.readableTimeStamp = function() {
    var theDate = new Date();
    var h = theDate.getHours();
    var m = theDate.getMinutes();
    var s = theDate.getSeconds();
...
// http://jsfiddle.net/Reyzn/42/
// Auteur: titi.... (mais inspiré de http://jsfiddle.net/Reyzn/ )
function dumpObj(obj, name, indent, depth, searchKey, searchValue, parentChild) {
              ...
              if (typeof obj == "object") {
                     ...
                     // if (name = "match: ") return parentChild;
                     indent += "\t";
                     for (var item in obj)
                     {
                           try {
                           ...
                           if (typeof child == "object") {
                                  parent = child;
                                  output += dumpObj(child, item, indent, depth + 1, searchKey, searchValue, parent);
                           } else {
                                  output += indent + item + ": " + child + "\n";
                                  if (child == searchValue) {
                                  output += name + " contient la valeur " + searchValue + "\n";
                                  output += "---------------------------------------------\n";
                                  output += dumpObj(parentChild, "match: ", indent, depth + 1);
                                  // return parentChild;
                                  } 
                           }
                     }
...
/*
var myTestObj =  {
"Offset":{"X":-20.7969943946,"Y":-3.02713963867, "Name":"PilierAAA"},
"Offset2":{"X":-20.7969943946,"Y":-3.02713963867, "Name":"PilierBBB"},
"Width":1485,"Stride":1488,"Height":1187,"Data":[0,0,0],"Resolution":0.019999999553}

var testObj = dumpObj(myTestObj, "testObj: ", " ", 4,"Name","PilierBBB");
alert (testObj);
/**/
