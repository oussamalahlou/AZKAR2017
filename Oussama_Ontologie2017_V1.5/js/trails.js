      ...
      htmlToDisplay+="<tr><td><center><label>Attribue</label></center></td><td><center><label>Valeur</label></center></td></tr>"
        ...
        for(desc in description){
          ...
          if(info[0]==="label")
            ...
            if(info[0] === "linkedToPOI"){
              htmlToDisplay+="<tr><td><label>linkedToScene</label></td><td><label>"+getPOIsScene(info[1])+"</label></td></tr>";
            }
            htmlToDisplay+="<tr><td><label>"+info[0]+"</label></td><td><label>"+info[1]+"</label></td></tr>";
          }
            
        }
        $('#modalBody').append("<div class='panel panel-success'><div class='panel-heading'><h4 class='panel-title'><a class='accordion-toggle' data-toggle='collapse' data-parent='#accordion' href='#"+TrailStepName+"'><b>"+TrailStepName+"</b></a></h4></div><div id='"+TrailStepName+"' class='panel-collapse collapse'><div class='panel-body'>"+htmlToDisplay+"</div></div></div>")
...
// fonction qui renvoi le nom scene correspondant à un point d'interêt
function getPOIsScene(POIName){
  for(p in listOfPOIs){
    if(listOfPOIs[p].poi === POIName)
      return listOfPOIs[p].scene;
  }
  ...
  $('#modalTitle').empty();
      ...
      while(moreDescription){
          if(description.charAt(i)=='|'){
              var propriete=s.substring(0,s.lastIndexOf(" : "));
              if(propriete=="label"){
                  $('#modalTitle').append('Détail du parcours  : <b>'+s.substring(s.lastIndexOf(" : ")+2)+'</b>');
              }    
              htmlToDisplay+="<tr><td><label>"+s.substring(0,s.lastIndexOf(" : "))+"</label></td><td><label>"+s.substring(s.lastIndexOf(" : ")+2)+"</label></td></tr>"             
              s="";
          } else{
